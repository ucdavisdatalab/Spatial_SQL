...
6. GitHub: In the repo's About section, add the reader URL and appropriate
   tags.

7. GitHub: In the repo's Settings page, enable GitHub pages. Set the branch to
   `main` and the directory to `docs/`.
   
8. `README.md`: Remove these template instructions, which end at the `#
...
4.  Run `knit.R` to regenerate the HTML files in the `docs/`. You can do this
    in the shell with `./knit.R` or in R with `source("knit.R")`. If you would
    like to include a PDF copy of the reader, add the flag `-p` (or `--pdf`).
    This will generate a `docs/_main.pdf` file, available for download via the
    menu bar in the reader.
