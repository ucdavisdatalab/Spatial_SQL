...
```SELECT id, ST_Length(geom) FROM flowlines;``` 
...
### Set the Projection
When you import your data into the database, it asked you what the SRID (EPSG Code) was for your data.  It's easy to forget to do this or to put in the wrong one if you're in a hurry.  If you discover that you've made a mistake, you don't need to re-import the table; you can set the SRID to the correct projection with an update command.  For our data it would look like this: 
...
To see the new table in the list, we'll need to refresh our *Browser* panel.  At the top of the *Browser* panel, click the *Refresh* button (it looks like two blue arrows chasing each other).  Now we can see the table, but it's just a table. Notice that the icon looks like a table, not a polygon.  The database doesn't seem to know the table is actually polygons.
...
This query will return a single column and row. Refresh your *Browser* again and the icon should change to a polygon.  Now we need to vacuum the database (yes, that sounds a little odd) to clean out any obsolete data that was leftover after we made these big changes.  Right click on the database name in the *Browser*, select *Compact Database (VACUUM)*.

Now we could add this to our map canvas to see the polygons.  Double click on the *watershedsUTM* table in the *Browser* and it should appear on the map canvas. If the transformation window pops up, read through the options and pick the best one. Note that we just reprojected the data, so it won't look too much different.

## Spatial Join
Let's go back to the query window and look at some more complex queries. 
